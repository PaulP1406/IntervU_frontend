...
export interface TechnicalFeedbackRequest {
    sessionId: string;
    questionId: string;
    userCode: string;
    hintsUsed: number;
...
export interface TechnicalFeedbackResponse {
    sessionId: string;
    hireAbilityScore: number;
    suggestions: string[];
    strengths: string[];
...
export async function getInterviewQuestions(
    ...
    console.log(
        "📤 [REQUEST] GET /api/interview-questions?sessionId=" + sessionId
    );

    const response = await fetch(
        `${API_BASE_URL}/api/interview-questions?sessionId=${sessionId}`
    );
...
export async function getInterviewFeedback(
    ...
    console.log("📦 [PAYLOAD]:", JSON.stringify(data, null, 2));
    console.log(
        "📊 [INFO] Total questions answered:",
        data.interviewQuestionsWithAnswers.length
    );
    ...
    console.log(
        "📊 [INFO] Feedback items:",
        result.interviewQuestionFeedback?.length || 0
    );
...
export async function getTechnicalQuestion(
    ...
    console.log(
        "📤 [REQUEST] GET /api/technical-question?difficulty=" + difficulty
    );

    const response = await fetch(
        `${API_BASE_URL}/api/technical-question?difficulty=${difficulty}`
    );
    ...
    const result = await response.json();
    console.log(
        "✅ [RESPONSE] Technical question received:",
        JSON.stringify(result, null, 2)
    );
    return result;
}
...
export async function executeCode(
    ...
    console.log(
        "📦 [PAYLOAD]:",
        JSON.stringify(
            { ...data, code: data.code.substring(0, 100) + "..." },
            null,
            2
        )
    );
...
export async function getHint(data: HintRequest): Promise<HintResponse> {
    ...
    console.log(
        "📦 [PAYLOAD]:",
        JSON.stringify(
            { ...data, userCode: data.userCode.substring(0, 50) + "..." },
            null,
            2
        )
    );
    ...
    const result = await response.json();
    console.log(
        "✅ [RESPONSE] Hint received:",
        JSON.stringify(result, null, 2)
    );
    return result;
}

// Submit technical feedback request
export async function submitTechnicalFeedback(
    data: TechnicalFeedbackRequest
): Promise<TechnicalFeedbackResponse> {
    const response = await fetch(`${API_BASE_URL}/api/technical-feedback`, {
        method: "POST",
        headers: {
            "Content-Type": "application/json",
        },
    ...
    if (!response.ok) {
        const error = await response.text();
        console.error("❌ [ERROR] Technical feedback failed:", error);
        throw new Error(error || "Failed to get technical feedback");
    }

    const result = await response.json();
    console.log(
        "✅ [RESPONSE] Technical feedback received:",
        JSON.stringify(result, null, 2)
    );
    return result;
}
